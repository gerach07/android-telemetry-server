...
if [ -f "$ROOT/system/bin/updater" ]; then
  echo "Pushing updater OTA helper to /data/local/tmp/updater.bin..."
  adb push "$ROOT/system/bin/updater" /data/local/tmp/updater.bin
  adb shell chmod 755 /data/local/tmp/updater.bin
fi

# OTA staging directory for updater (reporter + APKs before atomic install)
adb shell "mkdir -p /data/local/tmp/ota && chmod 777 /data/local/tmp/ota"
