...
# NOTE: script lives in `scripts/` so ROOT should be the repository root
# (parent of scripts). Previously ROOT pointed at scripts/, causing paths
# like "$ROOT/system/priv-app/..." to resolve to scripts/system/...,
# which didn't exist and made the installer fail to find APKs.
